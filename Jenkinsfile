pipeline {
  agent any
  environment {
    DOCKERHUB_USER = "junpeistyle"
    BUILD_HOST = "root@192.168.0.11"
    PROD_HOST = "root@192.168.0.12"
  ...
  stages {
    ...
    stage('Test') {
      steps {
        sh "docker -H ssh://${BUILD_HOST} container exec dockerkvs_apptest pytest -v -s test_app.py"
        sh "docker -H ssh://${BUILD_HOST} container exec dockerkvs_webtest pytest -v -s test_static.py"
        sh "docker -H ssh://${BUILD_HOST} container exec dockerkvs_webtest pytest -v -s test_selenium.py"
        sh "docker-compose -H ssh://${BUILD_HOST} -f docker-compose.build.yml down"
      }
    }
    stage('Register') {
